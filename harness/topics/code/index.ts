...
export function briefOf(progress: Progress, task: TaskFile["task"]): string {
    ...
    if (!r.ran) {
        ...
        return `Stage 5 of 6, run. The plugin is loaded in the forge (${(s.loaded?.types ?? []).join(", ")}). The task carries no telemetry: run the node in a document (forge.document_build with a spec that wires at least one of its inputs, a Logic.Time:timeline's value into it with segments that change: params {segments: a JSON string such as "[{\"from\":0,\"to\":120,\"value\":0.5},{\"from\":120,\"to\":1e9,\"value\":1}]", defaultValue: a number}, the connection from ["<timeline id>", "value"] to ["<node id>", "<input>"]; a name under "${task.id}/"; then forge.session_run on that name with a probe on one of its viewables) so that its outputs are seen over time; a document that leaves every input of the node unwired is refused.${refused("forge.session_run")}${refused("forge.document_build")}`;
    }
