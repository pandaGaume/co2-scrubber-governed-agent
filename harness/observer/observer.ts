...
export async function observe({ provider, broker, runtimeSlot = "twin", description, telemetry, attempts = 3 }: ObserveOptions): Promise<ObserveResult> {
    ...
    for (let n = 1, step = 1; n <= attempts; step++) {
        ...
        const input: PolicyFallbackInput = {
            ...
            state: {
                ...
                features: {
                    ...
                    ...(library.length ? { library: readsLeft > 0 ? `the documentation of the devices and of the station: library.list, library.search, library.read (${readsLeft} read(s) left)` : "no read left: hand over the request" } : {}),
        ...
        const check = checkTwinRequest(decision.invocation.input, { catalogueTypes: types, telemetryColumns: summary ? columns : undefined, vocabulary });
