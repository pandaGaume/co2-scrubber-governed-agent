...
describe("the workshop, through the broker", () => {
    ...
    it("the runtime on the twin: the catalogue, a spec validated, a document built by name into the task and run in the sandbox", async () => {
        ...
        // Two types produce a concentration in ppm since the substrate's atmosphere declares its species outputs (plugin-physics 0.1.2): the cabin and the atmosphere; asked for a cabin, the cabin comes first.
        assert.deepEqual(found.matches.map((m) => m.type).filter((t) => /cabin-air|^Physics\.Scene:atmosphere$/.test(t)).sort(), ["Physics.LifeSupport:cabin-air", "Physics.Scene:atmosphere"]);
