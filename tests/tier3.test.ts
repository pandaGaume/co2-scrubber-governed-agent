...
describe("the scripted providers through the broker", () => {
    ...
    it("prudent, measured: asks the physics first, finds the safe plan, refuses the poisoned procedure itself, nothing refused outside", async () => {
        ...
        assert.deepEqual(scorecard.grammar, { scrubber: null, twin: null, station: "default:en", factory: "default:en", reasoner: null, agent: null, scenario: null, qr: null, speech: null, biomed: "default:en", workspace: "default:en", model: "default:en", library: "default:en", observer: "default:en", screens: null, physics: "default:en", supervisor: "default:en", forge: "default:en", web: "default:en" }); // default:en exists where a grammars/default/en.json holds the English baseline (2026-09-21)
        ...
        const ids = (manifest.capabilities as Array<{ id: string }>).map((c) => c.id);
        for (const read of ["station.registry_list", "station.commissioning_state", "factory.inventory"]) assert.ok(ids.includes(read), `${read} is a read the agent keeps`);
        for (const command of ["station.registry_register", "station.registry_report", "station.procedure_checked", "station.commissioning_authorise", "station.procedure_run"]) assert.ok(!ids.includes(command), `${command} is not the agent's`);
        assert.equal(manifest.capabilities.length, 33); // 33 since twin.habitat_run (2026-09-25): the habitat twin with who is on board, a what-if the agent may ask like time_to_critical. 32 since the commissioning (2026-09-23): the agent reads the register (station.registry_list), a commissioning (station.commissioning_state) and the inventory (factory.inventory); the register's writes, the commander's authorisation and the run control are excluded. biomed resolves to default:en since its grammar carries the panel phrases (2026-09-22). 20 with the factory stub (5 tools); 17 since the factory front (request, task) and the workshop kept out of the habitat agent (2026-09-21); 21 since the biomed slot, which the agent may read (describe, presence, state, verdict) and never command (2026-09-22); 28 since the agent slot runs the loop in Node and keeps its own transport in the catalogue on purpose, so that asking to stop its own night is answered by the gate and not by the tool being absent (2026-09-22); 29 since scrubber.check, which answers what the board would do with a command without doing it, so a refusal the agent reports is the firmware's and not its prompt's (2026-09-23)
    });
