...
|---|---|
| `factory.request { objective, observations, data, constraints?, topics?: ["graph", "onnx"] \| "auto", budget?, run? }` | crée la tâche (le fichier de tâche de la section 5), ouvre l'atelier, écrit les données reçues (`data[].rows` ou `data[].text`, sha256 dans le fichier de tâche), lance le runner (local : en processus, fait le 21 septembre ; à distance : `nebius ai job create`) ; rend `{ taskId, state, started, builder, data }` ; `run: false` ouvre la tâche sans la lancer |
...
**Duplication temporaire avec `tier3/`, à noter et à résorber.** *(Note du 21 septembre : la règle a été durcie avant F3, voir `harness/README.md` : rien n'est copié, ce qui est générique monte dans `harness/` et `tier3/` l'importe ; pour F4, le graphe est monté dans `harness/lib/flow.ts`, `createAgent` et `buildCapabilities` ont pris leur forme générale dans `harness/core` et `tier3/` les appelle avec ses propres services. Le texte ci-dessous est celui de la spécification d'origine.)* Le Tier 3 a déjà, sous `tier3/`, ce dont l'usine a besoin : le client du broker (`lib/broker.ts`, `lib/mcp-http.ts`), l'enregistrement des outils du broker comme capacités (`lib/capabilities.ts`), la construction de l'agent (`agent.ts`), les adaptateurs de modèles (`providers/anthropic.ts`, `openai-compatible.ts`, `llm-common.ts`, `reasoner.ts`, `compose.ts`) et le constructeur scripté (`providers/scripted.ts`). Règle pour F3 et F4 :
