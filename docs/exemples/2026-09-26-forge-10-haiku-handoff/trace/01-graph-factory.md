# 01-graph-factory: task t-2026-09-26-0014

- steps: 3, model calls: 3, model: claude-haiku-4-5-20251001, tokens in/out: 6,069 / 1,374
...
| 3 | plan | 14 steps, 200 runs | task.plan -> completed | planAccepted, candidateEvaluated, candidateHeld | CONSISTENT |  |  | task.plan: plan refused: missing capability "leak_co2 (MassFlow, kg/s)", contract: type "Ph | 0 | 5,893 |

## Step 1: workspace.read (fallback) -> completed

- node: phase plan; last capability before: ; 1269 ms
...
- node: ?; last capability before: none; 7818 ms
- failed: plan refused: missing capability "leak_co2 (MassFlow, kg/s)", contract: type "Physics.Habitat:leak" is not named under "Generated." (as in "Generated.Habitat:leak"): a generated node is; leave type out to let the code factory name it; missing capability "leak_co2 (MassFlow, kg/s)" is not the name of a required output: write required_output exactly as the objective names it, "predicted_co2" or "leak_co2", nothing added
  ...
  "missing_capabilities": [
    {
      ...
      "quantity": "MassFlow",
      "unit": "kg/s",
      "reason": "The catalogue contains no leak node; the task hypothesis states a seal of the Lab leaks and requires leak_co2 output, but every CO2 sink in the catalogue is a scrubber. A leak node must model mass flow out of the atmosphere based on a pressure differential or opening fraction.",
      "topic": "code",
      "contract": {
...
### Model call: claude-haiku-4-5-20251001, 1,932 in / 778 out, 7785 ms

<details><summary><strong>Response, raw</strong> (3,041 characters, folded)</summary>

```text
I'll start by submitting the plan with the node types from the reference graph "habitat" that are listed in the shelf. The task requires producing `predicted_co2` (Concentration, ppm) and `leak_co2` (MassFlow, kg/s), and the shelf provides a reference graph with known node types and variables.

Based on the shelf information, I can see:
- The reference graph "habitat" uses these node types: Control.Sim:rk4-solver, DSP.Sensor:transducer, Logic.Time:timeline, Physics.Habitat:crew, Physics.Habitat:fan, Physics.Habitat:filter, Physics.Habitat:person, Physics.Habitat:scrubber, Physics.Particulate:lunar_dust, Physics.Scene:atmosphere, Physics.Scene:atmosphere-gate, Physics.Scene:moon
- Variables are defined with their status (fitted, known, device, band)
- The hypothesis notes that "a seal of the Lab leaks; no node of the catalogue expresses a leak"

This means I need to declare a missing capability for the leak node, since the catalogue has no leak node but the task requires `leak_co2` output.
...
- node: phase plan -> phase build; last capability before: workspace.read; 4454 ms
- rationale: I need to resubmit the plan with the correct format for the missing capability. The harness rejected my previous attempt because:

1. The type name should be left out (let the code factory name it)
2. The required_output must match exactly: "leak_co2" not "leak_co2 (MassFlow, kg/s)"
...
### Model call: claude-haiku-4-5-20251001, 2,475 in / 531 out, 4403 ms

**Response, raw**

```text
I need to resubmit the plan with the correct format for the missing capability. The harness rejected my previous attempt because:
